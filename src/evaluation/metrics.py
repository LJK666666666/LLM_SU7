...
def compute_picp(y_true, y_pred, y_std, confidence=0.95, sigma_space='log'):
    ...
    参数:
        ...
        sigma_space: sigma所在空间 ('log' 或 'original')
...
def compute_mpiw(y_pred, y_std, confidence=0.95, sigma_space='log'):
...
def evaluate(y_true, y_pred, prefix='', y_std=None, sigma_space='log'):
    """综合评估函数
    ...
    if y_std is not None:
        ...
        metrics[f'{prefix}PICP@95%'] = compute_picp(y_true, y_pred, y_std, confidence=0.95, sigma_space=sigma_space)
        metrics[f'{prefix}MPIW@95%'] = compute_mpiw(y_pred, y_std, confidence=0.95, sigma_space=sigma_space)
